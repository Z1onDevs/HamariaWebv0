...
export function DNAHelix({ scrollProgress = 0 }: DNAHelixProps) {
  ...
  useEffect(() => {
    ...
    const drawHelix = () => {
      ...
      // Enhanced glow backdrop - using bright blue from website palette
      if (scrollProgress > 0.2) {
        points.forEach((point, i) => {
          if (i % 3 === 0) {
            ...
            gradient.addColorStop(0, `rgba(74, 144, 226, ${depthOpacity * glowIntensity * 0.4})`)
            gradient.addColorStop(0.5, `rgba(74, 144, 226, ${depthOpacity * glowIntensity * 0.15})`)
      ...
      // Draw base pairs (connecting lines) with enhanced thickness - using bright blue
      points.forEach((point, i) => {
        ...
        ctx.strokeStyle = `rgba(74, 144, 226, ${opacity * baseOpacity})`
        ...
        if (scrollProgress > 0.3 && i % 2 === 0) {
          ...
          ctx.shadowColor = `rgba(74, 144, 226, ${scrollProgress * 0.6})`
          ctx.stroke()
          ctx.shadowBlur = 0
        }
      })

      // Draw the two helical strands with enhanced definition - using bright blue and gold
      for (let strand = 0; strand < 2; strand++) {
        // Draw glow layer first
        if (scrollProgress > 0.2) {
          ctx.shadowBlur = 15 * scrollProgress
          ctx.shadowColor = `rgba(74, 144, 226, ${scrollProgress * 0.8})`
        }
        ...
        points.forEach((point, i) => {
          ...
          // Draw nodes with enhanced size and glow - bright blue
          const nodeSize = 3 + intensity * 2
          const nodeOpacity = 0.6 + intensity * 0.4
          ctx.fillStyle = `rgba(74, 144, 226, ${opacity * nodeOpacity})`
          ctx.beginPath()
          ctx.arc(x, y, nodeSize, 0, Math.PI * 2)
          ctx.fill()

          // Add bright core to nodes when scrolling - gold accent
          if (scrollProgress > 0.4) {
            ctx.fillStyle = `rgba(201, 169, 97, ${opacity * scrollProgress * 0.6})`
            ctx.beginPath()
        ...
        ctx.strokeStyle = `rgba(74, 144, 226, ${strandOpacity})`
        ctx.lineWidth = 2 + intensity * 2
        ctx.stroke()
        ctx.shadowBlur = 0
      }

      // Draw energy particles flowing along the helix - using gold and light blue
      particlesRef.current = particlesRef.current.filter(particle => {
        ...
        // Draw particle with glow - gold accent
        const particleSize = 4 + scrollProgress * 3
        const gradient = ctx.createRadialGradient(x, yPos, 0, x, yPos, particleSize * 3)
        gradient.addColorStop(0, `rgba(201, 169, 97, ${depthOpacity * particle.life * 0.9})`)
        gradient.addColorStop(0.4, `rgba(158, 196, 247, ${depthOpacity * particle.life * 0.5})`)
        gradient.addColorStop(1, "rgba(74, 144, 226, 0)")
        ...
        ctx.fillStyle = `rgba(158, 196, 247, ${depthOpacity * particle.life * 0.8})`
        ctx.beginPath()
